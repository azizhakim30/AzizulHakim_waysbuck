...
const Detailproduct = () => {
  ...
  const Transaction = { email, topingCheck, idProduct, totalPrice }
  ...
  const handleOnSubmit = (e) => {
    ...
    if (DataTransaction == null) {
      ...
      localStorage.setItem("TRANSACTION", JSON.stringify(arrTransaction))
    } else {
      for (let i = 0; i < DataTransaction.length; i++) {
        arrTransaction.push(DataTransaction[i])
      }
      arrTransaction.push(Transaction)
      localStorage.setItem("TRANSACTION", JSON.stringify(arrTransaction))
    }
